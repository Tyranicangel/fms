...
class PeshiController extends Controller {

	public function get_peshi_users(){
		$session=Session::where('refresh_token','=',Request::header('JWT-AuthToken'))->first();
		return User::where('id','!=',$session->user_id)->where('user_role','!=','2')->where('user_role','!=','1')->where('user_role','!=','9')->select('id','user_role','user_desc')->get();
	}

	public function get_peshi_files(){
		$session=Session::where('refresh_token','=',Request::header('JWT-AuthToken'))->first();
	...
	public function peshi_close_file(){
		$session=Session::where('refresh_token','=',Request::header('JWT-AuthToken'))->first();
		$usermain=User::where('id','=',$session->user_id)->first();
		$date=Carbon::now();
	...
	public function peshi_frwd_file(){
		$session=Session::where('refresh_token','=',Request::header('JWT-AuthToken'))->first();
		$usermain=User::where('id','=',$session->user_id)->first();
		$date=Carbon::now();
		$tid=Request::input('tid');
	...
	public function peshi_generate_frn(){
		$date=Carbon::now();
		$file=Request::all();
		$session=Session::where('refresh_token','=',Request::header('JWT-AuthToken'))->first();
		$nf=new File;
