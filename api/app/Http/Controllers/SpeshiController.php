...
class SpeshiController extends Controller {

	public function get_speshi_users(){
		$session=Session::where('refresh_token','=',Request::header('JWT-AuthToken'))->first();
		return User::where('id','!=',$session->user_id)->where('user_role','!=','2')->where('user_role','!=','1')->where('user_role','!=','7')->select('id','user_role','user_desc')->get();
	}

	public function get_speshi_files(){
		$session=Session::where('refresh_token','=',Request::header('JWT-AuthToken'))->first();
	...
	public function speshi_close_file(){
		$session=Session::where('refresh_token','=',Request::header('JWT-AuthToken'))->first();
		$usermain=User::where('id','=',$session->user_id)->first();
		$date=Carbon::now();
	...
	public function speshi_frwd_file(){
		$session=Session::where('refresh_token','=',Request::header('JWT-AuthToken'))->first();
		$usermain=User::where('id','=',$session->user_id)->first();
		$date=Carbon::now();
		$tid=Request::input('tid');
	...
	public function speshi_generate_frn(){
		$date=Carbon::now();
		$file=Request::all();
		$session=Session::where('refresh_token','=',Request::header('JWT-AuthToken'))->first();
		$nf=new File;
