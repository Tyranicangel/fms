...
class SpecialsecController extends Controller {

	public function get_special_sec_files(){
	...
	public function get_special_sec_users(){
		$session=Session::where('refresh_token','=',Request::header('JWT-AuthToken'))->first();
		return User::where('user_role','=','8')->select('id','user_role','user_desc')->get();
	}

	public function special_sec_frwd_file(){
		$session=Session::where('refresh_token','=',Request::header('JWT-AuthToken'))->first();
		$usermain=User::where('id','=',$session->user_id)->first();
		$secid=$usermain->section_id;
		$date=Carbon::now();
