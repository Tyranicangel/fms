app.controller('PeshiController', ['$scope','$http','$rootScope','Dates','Logging', function ($scope,$http,$rootScope,Dates,Logging) {
	...
	$http({
		method:'GET',
		url:$rootScope.requesturl+'/get_peshi_users',
	...
	$http({
		method:'GET',
		url:$rootScope.requesturl+'/get_peshi_files',
		headers:{'JWT-AuthToken':localStorage.fmstoken}
	}).success(function(result){
		$rootScope.showloader=false;
	...
	$scope.frwd_file=function(){
		...
		else
		{
			$http({
				method:'POST',
				url:$rootScope.requesturl+'/peshi_frwd_file',
				headers:{'JWT-AuthToken':localStorage.fmstoken},
	...
	$scope.close_file=function(){
		...
		$http({
			method:'POST',
			url:$rootScope.requesturl+'/peshi_close_file',
			headers:{'JWT-AuthToken':localStorage.fmstoken},
...
app.controller('PeshiCreateFileController', ['$scope','$http','$rootScope','Logging', function ($scope,$http,$rootScope,Logging) {
	$rootScope.showloader=true;
	...
	$scope.generate_frn=function(){
		...
		$http({
			method:'POST',
			url:$rootScope.requesturl+'/peshi_generate_frn',
			headers:{'JWT-AuthToken':localStorage.fmstoken},
			data:$scope.file
		}).success(function(result){
			$rootScope.showloader=false;
