app.controller('OfficerController', ['$scope','$http','$rootScope','Dates','Logging', function ($scope,$http,$rootScope,Dates,Logging) {
	...
	$http({
		method:'GET',
		url:$rootScope.requesturl+'/get_officer_users',
	...
	$http({
		method:'GET',
		url:$rootScope.requesturl+'/get_officer_files',
		headers:{'JWT-AuthToken':localStorage.fmstoken}
	}).success(function(result){
		$rootScope.showloader=false;
	...
	$scope.frwd_file=function(){
		...
		else
		{
			$http({
				method:'POST',
				url:$rootScope.requesturl+'/officer_frwd_file',
				headers:{'JWT-AuthToken':localStorage.fmstoken},
