app.controller('SecController', ['$scope','$http','$rootScope','Dates','Logging', function ($scope,$http,$rootScope,Dates,Logging) {
	$scope.Dates=Dates;
	$scope.userselect=6;
	$scope.remarks="";
	$http({
		method:'GET',
		url:$rootScope.requesturl+'/get_sec_users',
	...
	$http({
		method:'GET',
		url:$rootScope.requesturl+'/get_sec_files',
		headers:{'JWT-AuthToken':localStorage.fmstoken}
	}).success(function(result){
		$rootScope.showloader=false;
	...
	$scope.frwd_file=function(){
		...
		else
		{
			$http({
				method:'POST',
				url:$rootScope.requesturl+'/sec_frwd_file',
				headers:{'JWT-AuthToken':localStorage.fmstoken},
